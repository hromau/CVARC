...
namespace ReplayDebugger
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var GameSettings =
                Serializer.Deserialize<GameSettings>(Encoding.UTF8.GetString(settingsFile.OpenReader().ReadToEnd()));
            ...
            var worldState =
                Serializer.Deserialize<JObject>(Encoding.UTF8.GetString(settingsFile.OpenReader().ReadToEnd()));
        ...
        private static Dictionary<string, List<JObject>> ParseCommands(ZipEntry replayFile)
        {
            ...
            while (true)
            {
                ...
                if (line == null) break;
                var entry = Serializer.Deserialize<GameLogEntry>(line);

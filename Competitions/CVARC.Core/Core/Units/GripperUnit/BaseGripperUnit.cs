...
namespace CVARC.V2
{
    ...
    public abstract class BaseGripperUnit<TRules> : IUnit
    {
        ...
        public virtual Frame3D RobotLocation { get { return actor.World.GetEngine<ICommonEngine>().GetAbsoluteLocation(actor.ObjectId); } }
        
        public virtual GrippingAvailability GetAvailability(string objectId)
        {
            var engine = actor.World.GetEngine<ICommonEngine>();
            if (!engine.ContainBody(objectId)) return null;
            var gripperLocation = RobotLocation.Apply(GrippingPoint);
            var objectLocation = engine.GetAbsoluteLocation(objectId);
